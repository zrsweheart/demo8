...
@app.route('/fenge', methods=['POST'])
def show():  # 接收前端上传的一个分片
    patientname = request.form.get('patientname')  # 获取文件的唯一标识符
    patientid= request.form.get('patientid')  # 获取该分片在所有分片中的序号
    return rt('./result2.html', patientid=patientid,patientname=patientname)
